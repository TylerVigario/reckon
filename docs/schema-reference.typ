...
#callout(tone: "note")["per site and per client. we talked about this. reoccurings should be per site or client." — 18 Aug. One shape: a basis plus the locations it covers.  \$200 a month per site — "It's 200 for Traver and 200 for kettleman. (Per site)" — 2 Sep. Bravo hold two subscriptions, one per main site, and their agreement is unlimited: "\$400 a month for remote support (unlimited)".  "the default remote support is 2 hours" — 9 Sep; whether a new agreement starts from it is open. The pool is "set at the client level vs the site level", so the meter sits here rather than per location.  "allotment used and they call. bill per minute at the going rate. also can be set as no-charge or deny work" — 9 Sep.  "retainer does meter but bravo will show infinite right now" — 18 Aug.  "its from the 1st of a the month til the end of a month. billed for upcoming months usage. this month will be given freely" — 23 Sep. A period is charged when it begins, and can be given: covered, charged nothing, on purpose.  "whats the difference between on-site and remote? why are they categorical instead of universal?" — 23 Sep. The split answered one question, which hours come out of a retainer, and answered it by kind. agreement\_service names the services an agreement covers, each with its own allotment, so an on-site retainer is as easy as a remote one and an hour on a service not named is billed.  "allotment for a service shouldnt even a part of its service configuration. that should be per client and/or per site" — 24 Sep. agreement\_service is the only home an allotment has: for the whole client (flat) or per site covered (per\_location).  [claude] A capped pool is drawn in the order hours were worked, within an agreement\_period — a charged period. Hours it covers bill nothing by the hour and are paid as a share of that period's charge.]
#v(4pt)
...
#sheet(
  ...
  [Whether a new agreement's allotment starts at two hours], [\*"the default remote support is 2 hours"\* — 9 Sep — had its home on the service until 0023. With allotments set per client or per site, whether it is the figure a new agreement starts from is open.],
  [What share of a retainer is paid for covered time, away from Bravo], [Bravo is the only retainer; another would pay nobody for covered time until a rule says what share.],
